...
# Check command-line arguments. If no argument is provided, just run the
# test file.
try:
    opts, args = getopt.getopt(sys.argv[
                               1:], "d:n:m:", ["depth", "numgames", "fromdepth"])
...
try:
    conn = psycopg2.connect(
        "dbname='ficsdb' user='dfslezak' host='calamaro.exp.dc.uba.ar' password='23571113'")
    cursor = conn.cursor()
except:
...
for i in range(numgames):
    ...
    # statement = "SELECT min(id_game),max(id_game) FROM games WHERE
    # games.increment="+str(increment)+" AND
    # games.total_time="+str(total_time)
    ...
    # Get unanalysed game.
    # statement = "SELECT id_game, get_game(id_game) FROM games WHERE
    ...
    statement = "SELECT id_game, get_game(id_game) FROM unanalyzed_games_tz WHERE id_game>"+str(random_game)+" LIMIT 1;"
    cursor.execute(statement)
    result = cursor.fetchone()
    ...
    # Get unanalysed game.
    statement = "SELECT id_move, move_number,w_b FROM moves WHERE game=" + \
        str(id_game) + "ORDER BY move_number,w_b"
    cursor.execute(statement)
    ...
    # Check that stockfish analysed all moves and generate SQL query for
    # insertion
        ...
        for move, score_array in scores:
            ...
            if iterator > 0:
                if (iterator % 2 == 0):
                    ...
                    sign = (-1.0)
                delta_score = sign*(float(score_array[
                                    5])-float(scores[iterator-1][1][6]))
                sql_move = ','
            else:
    ...
    statement = "SET enable_seqscan=false; SELECT check_stockfish_analysed_game(" + str(
        id_game) + ");"
    cursor.execute(statement)
    result = cursor.fetchone()
