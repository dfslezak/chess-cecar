    ...
    if move_type[-1] == "#":
        ...
        while not end_string in line:
            ...
            if search_string in line and "mate" in line:
                ...
                line_depth = int(line[2])
                if line_depth >= fromdepth:
                    if (int(line[line.index("mate")+1]) < 0):
                        sign = -1
                    else:
                        sign = 1
...
def analyzeGame(pgntext, depth, fromdepth):
    ...
    # end_code =
    # str(pgn_splitted[pgn_splitted.index('[EndCode')+1].strip('"]'))
    ...
    ## ************ Start Stockfish ************
    stockfish = Popen("./stockfish.out", bufsize=0, stdin=PIPE,
                      stdout=PIPE, stderr=STDOUT, universal_newlines=True, shell=False)
    # Set uci interface
    cmd = "uci\n"
    stockfish.stdin.write(cmd)
    ...
    for i in range(len(moves)):
        ...
        # If this is the last move, check the end_code. If it is "Sta" then
        # score = 0
        if (i == len(moves)-1) and (('drawn' in pgntext) or ('Neither player' in pgntext)):
            for i in range(fromdepth, depth+1):
                score_array[i-fromdepth] = 0
        else:
            score_array = analyzeMove(
                stockfish, move, white_turn, depth, fromdepth)
    ...
    # print 'Time control: PGN_EXTRACT='+str(end_pgnExtract-start_ag)+' ---
    # STOCKFISH_FORK='+str(end_fork-end_pgnExtract)+' --- ANALYSIS='+str
    # (end_analysis-end_fork)
